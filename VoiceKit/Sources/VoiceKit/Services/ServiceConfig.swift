...
///   2. Environment variables (VOICE_SERVICE_URL, VOICE_API_KEY)
///
...
public final class ServiceConfig: @unchecked Sendable {
    ...
    /// Base URL of the VoiceService zone.
    ///
    /// Checks Keychain first, then falls back to the VOICE_SERVICE_URL
    /// environment variable, then to localhost for local development.
    public var baseURL: String {
        if let url = keychain.serviceURL(), !url.isEmpty {
            return url
        }
        return ProcessInfo.processInfo.environment["VOICE_SERVICE_URL"]
            ?? "http://localhost:8000"
    }
    ...
    /// API key from the VOICE_API_KEY environment variable (dev workflow).
