...
struct AudioLevelAnalyzerTests {
    ...
    @Test("Raw PCM rmsLevel matches the WAV-encoded rmsLevel for the same samples")
    func rawMatchesWav() {
        let samples: [Int16] = (0..<1600).map { $0 % 2 == 0 ? Int16(3000) : Int16(-3000) }
        let viaRaw = AudioLevelAnalyzer.rmsLevel(pcm16: rawPCM(samples))
        let viaWav = AudioLevelAnalyzer.rmsLevel(of: makeBuffer(samples: samples))
        // Floating-point computation order is identical; values should match exactly.
        #expect(viaRaw == viaWav)
    }
