...
struct DictationProviderTests {
    ...
    // MARK: - FreeFlowServiceDictationProvider request construction

    @Test("Request has correct URL")
    func requestURL() throws {
        let provider = FreeFlowServiceDictationProvider(
    ...
    func streamingProviderExplicitOverrides() {
        ...
        let provider = FreeFlowServiceStreamingProvider(
...
struct DictationProviderKeychainTests {
    ...
    func dictationProviderDynamicBaseURL() throws {
        ...
        let provider = FreeFlowServiceDictationProvider(config: config)
    ...
    func streamingProviderDynamicConfig() {
        ...
        // Create provider before any Keychain credentials exist.
        let provider = FreeFlowServiceStreamingProvider(config: config)
