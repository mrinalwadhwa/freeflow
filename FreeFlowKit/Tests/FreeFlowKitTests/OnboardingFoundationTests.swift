...
struct KeychainServiceTests {
    ...
    private func makeKeychain() -> KeychainService {
        ...
        return KeychainService(service: "computer.autonomy.freeflow.test.\(id)")
    }
    ...
    func sharedService() {
        let serviceName = "computer.autonomy.freeflow.test.shared.\(UUID().uuidString.prefix(8))"
...
struct AuthClientTests {
    ...
    func validateSessionPlaceholder() async throws {
        let body = """
            {
                ...
                "user": {
                    "id": "usr_ph",
                    "name": "Invited User",
                    "email": "abc123@placeholder.freeflow.local",
                    "emailVerified": false
                }
            }
            """.data(using: .utf8)!
        ...
        #expect(result.email == "abc123@placeholder.freeflow.local")
...
struct CapabilitiesServiceTests {
    ...
    func checkSuccess() async throws {
        let body = """
            {
                ...
                "appcast_url": "https://freeflow.example.com/appcast.xml"
            }
            """.data(using: .utf8)!
        ...
        #expect(caps.appcastUrl == "https://freeflow.example.com/appcast.xml")
