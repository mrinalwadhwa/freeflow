...
/// A minimal WebSocket server that speaks the FreeFlowServiceStreamingProvider
...
/// `FreeFlowServiceStreamingProvider`. These tests run a local WebSocket
...
final class BackupConnectionTests: XCTestCase {
    ...
    override func setUp() async throws {
        try XCTSkipUnless(
            ...
            "Slow backup connection tests skipped (set FREEFLOW_TEST_SLOW=1 to run)")
    ...
    private func makeProvider() -> FreeFlowServiceStreamingProvider {
        FreeFlowServiceStreamingProvider(
    ...
    private func runSession(on provider: FreeFlowServiceStreamingProvider) async throws -> String {
